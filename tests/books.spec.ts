...
test.describe('Books Page', () => {
  ...
  test('PDF books display thumbnails with fallback', async ({ page }) => {
    ...
    if (cardCount > 0) {
      const pdfCards = bookCards.filter({ has: page.locator('.book-card-badge:has-text("PDF")') });
  ...
  test('EPUB books display icon without thumbnail', async ({ page }) => {
    ...
    if (cardCount > 0) {
      const epubCards = bookCards.filter({ hasNot: page.locator('.book-card-badge:has-text("PDF")') });
